...
rules. It covers most of the use-cases and concepts of rules.
...
export const rule = {
  create(context) {
    return {
      OperationDefinition(node) {
        const schema = requireGraphQLSchema('my-rule', context)
...
call `requireGraphQLSchema` - it will validate that the schema is loaded.
