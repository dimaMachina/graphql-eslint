...
You will also need to add `"graphql"` to `eslint.validate` property in `.vscode/settings.json`.
If you don't have `eslint.validate`, you will need to create it and add all languages you want ESLint to `validate`.

```diff filename=".vscode/settings.json"
{
   "eslint.validate": [
     "javascript",
     "typescript",
